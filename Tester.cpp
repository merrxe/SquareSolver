...
int OneTest (SquareTestData dataOneTest)
    ...
    if (!(IsEqual (x1, x1_ref)  && IsEqual (x2, x2_ref) && nRoots == nRoots_ref))
        {
        printf ("Ïğè ğåøåíèè êâàäğàòíîãî óğàâíåíèÿ ñ êîıôôèöèåíòàìè a = %.2f, b = %.2f, c = %.2f ïîëó÷åíû êîğíè %.2f è %.2f, îæèäàåìûå êîğíè %.2f è %.2f. \n", a, b, c, x1, x2, x1_ref, x2_ref);
        return 1;
        }
    return 0;
    }
